	...
	var commands = {
		"subscribe": function (user, args) {
			if(subscribed.hasOwnProperty(user) && !args.length) {
				...
				sendTo("You have been successfully subscribed. Currently subscribed: " + subscribed._length, user);
			}
		},
		"unsubscribe": function (user, args) {
			if(!subscribed.hasOwnProperty(user)) {
				...
				if(!args.length) {
					...
					sendTo("You have been successfully unsubscribed. Currently subscribed: " + subscribed._length, user);
				} else {
